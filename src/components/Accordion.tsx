            ...
            >
                <AccordionPrimitive.Header
                    ...
                    <AccordionPrimitive.Trigger
                        className={cn(
                            "flex flex-1 gap-8 items-center justify-between py-4 font-medium transition-all text-2xl",
...
const AccordionContent = React.forwardRef<React.ComponentRef<typeof AccordionPrimitive.Content>, AccordionContentProps>(({ className, children, ...props }, ref) => {
    return (
        <AccordionPrimitive.Content
            className={cn(
                "overflow-hidden text-xl data-[state=open]:bg-gray-300/40 px-4 rounded-b-md",
